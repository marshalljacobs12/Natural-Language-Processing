...
def cossim_sparse(v1,v2):
    ...
    cosineSimilarity = dotProduct / denominator
...
def show_nearest(word_2_vec, w_vec, exclude_w, sim_metric):
    ...
    scores = {}
    for k, v in word_2_vec.iteritems():
        if k not in exclude_w:
            scores[k] = sim_metric(w_vec, v)

    mostSimilar = sorted(scores.iteritems(), key=lambda(k,v):(v,k), reverse=True)[:10]
